...
Введи команду /info чтобы узнать все команды) 
""")
    info(message)
...
def info(message):
    bot.send_message(message.chat.id, "Здесь будут появляться новые проекты, пока новых проектов нету ")
...
def info(message):
    bot.send_message(message.chat.id, """
Связаться с разработчиком - @fedorzavg
""")
...
def description_project(message, data):
    ...
    bot.send_message(message.chat.id, "Введите ссылку на проекта")
